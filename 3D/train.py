...
def train(opt):
    ...
    curr_states = curr_states.type(torch.FloatTensor)

    if torch.cuda.is_available():
        curr_states = curr_states.cuda()
    curr_episode = 0
    while True:
        print("episode:", curr_episode)
        ...
        for _ in range(opt.num_local_steps):
            ...
            # print("curr_states.dtype:", curr_states.dtype)
            ...
            # if torch.cuda.is_available():
            #    print("action:", action.cpu())
            #    envs.step(action.cpu())
            #    #[agent_conn.send(("step", act)) for agent_conn, act in zip(envs.agent_conns, action.cpu())]
            # else:
            #    [agent_conn.send(("step", act)) for agent_conn, act in zip(envs.agent_conns, action)]

            state, reward, done, info = envs.step(action.cpu().detach().numpy())
        ...
        print("old_log_policies:", len(old_log_policies))
